...
void main(){
  ...
  vec4 texSample=texture(u_tex,iC);float dp=texSample.r,shapeMask=texSample.a;
  ...
  float vs=shapeMask;vs*=bM(iC,.01);
  float fr=pow(1.-cv,u_fresnel)*.3;vs=min(vs+fr*vs,1.);
  float mT=an*.0625;
  ...
  float rB=clamp(1.-cv,0.,1.);float fl=ag.x+sl;
  fl+=noiseVal*sl*u_distort*edgeFactor;fl*=mix(1.,1.-dp*.5,u_contour);fl-=dp*u_contour*.8;
  ...
  rO*=u_refract*u_chroma;bO*=u_refract*u_chroma;
  float sf=u_blur;
  float rC=mG(hi.r,lo.r,fract(fl+rO),sf+.018+u_refract*cv*.025,cv);
  float gC=mG(hi.g,lo.g,fract(fl),sf+.008/max(.01,1.-sl),cv);
  float bC=mG(hi.b,lo.b,fract(fl-bO),sf+.008,cv);
...
function processImage(img: HTMLImageElement): ImageData {
  ...
  const scale = w > h ? (w > MAX ? MAX/w : w < MIN ? MIN/w : 1) : (h > MAX ? MAX/h : h < MIN ? MIN/h : 1)
  ...
  for (let i = 0; i < size; i++) {
    const px = i * 4, r = data[px], g = data[px+1], b = data[px+2], a = data[px+3]
  ...
  for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) {
    ...
    if (x === 0 || x === w-1 || y === 0 || y === h-1 || !shape[idx-1] || !shape[idx+1] || !shape[idx-w] || !shape[idx+w]) boundary[idx] = 1
  }
  ...
  for (let iter = 0; iter < 200; iter++) {
    for (let y = 1; y < h-1; y++) for (let x = 1; x < w-1; x++) {
      ...
      const sum = (shape[idx+1] ? u[idx+1] : 0) + (shape[idx-1] ? u[idx-1] : 0) + (shape[idx+w] ? u[idx+w] : 0) + (shape[idx-w] ? u[idx-w] : 0)
      u[idx] = omega * (C + sum) / 4 + (1 - omega) * u[idx]
    }
  }
  let maxV = 0; for (let i = 0; i < size; i++) if (u[i] > maxV) maxV = u[i]; if (!maxV) maxV = 1
  const out = ctx.createImageData(w, h)
  for (let i = 0; i < size; i++) {
    const px = i * 4, gray = Math.round(255 * (1 - (u[i]/maxV) ** 2))
...
export default function MetallicPaint({
  ...
  liquid = 0.75, speed = 0.3, brightness = 2, contrast = 0.5, angle = 0,
  fresnel = 1, lightColor = '#ffffff', darkColor = '#000000',
  patternSharpness = 1, waveAmplitude = 1, noiseScale = 0.5,
  chromaticSpread = 2, mouseAnimation = false, distortion = 1,
  contour = 0.2, tintColor = '#00f5ff'
}: MetallicPaintProps) {
  ...
  const animTimeRef = useRef(0), lastTimeRef = useRef(0)
  ...
  const initGL = useCallback(() => {
    ...
    const gl = canvas.getContext('webgl2', { antialias: true, alpha: true }); if (!gl) return false
    ...
    const vs = compile(vertexShader, gl.VERTEX_SHADER), fs = compile(fragmentShader, gl.FRAGMENT_SHADER)
    if (!vs || !fs) return false
    const prog = gl.createProgram(); if (!prog) return false
    ...
    const cnt = gl.getProgramParameter(prog, gl.ACTIVE_UNIFORMS)
    for (let i = 0; i < cnt; i++) { const info = gl.getActiveUniform(prog, i); if (info) uniforms[info.name] = gl.getUniformLocation(prog, info.name) }
    ...
    glRef.current = gl; programRef.current = prog; uniformsRef.current = uniforms
    return true
  }, [])

  const uploadTexture = useCallback((imgData: ImageData) => {
    const gl = glRef.current, u = uniformsRef.current; if (!gl) return
  ...
  useEffect(() => {
    if (!initGL()) return
    const canvas = canvasRef.current, gl = glRef.current; if (!canvas || !gl) return
    const side = 1000 * devicePixelRatio
    canvas.width = side; canvas.height = side; gl.viewport(0, 0, side, side)
    setReady(true)
  ...
  useEffect(() => {
    const gl = glRef.current, u = uniformsRef.current; if (!gl || !ready) return
    ...
    const li = hexToRgb(lightColor), da = hexToRgb(darkColor), ti = hexToRgb(tintColor)
    gl.uniform3f(u.u_lightColor, li[0], li[1], li[2]); gl.uniform3f(u.u_darkColor, da[0], da[1], da[2])
    ...
    gl.uniform3f(u.u_tint, ti[0], ti[1], ti[2])
  ...
  useEffect(() => {
    ...
    const gl = glRef.current, u = uniformsRef.current, canvas = canvasRef.current
    if (!gl || !canvas) return
    ...
    const render = (time: number) => {
      ...
      if (mouseAnimRef.current) { mouse.x += (mouse.targetX - mouse.x) * 0.08; mouse.y += (mouse.targetY - mouse.y) * 0.08; animTimeRef.current = mouse.x * 3000 + mouse.y * 1500 }
      else animTimeRef.current += delta * speedRef.current
      gl.uniform1f(u.u_time, animTimeRef.current); gl.drawArrays(gl.TRIANGLE_STRIP, 0, 4)
      rafRef.current = requestAnimationFrame(render)
    }
    lastTimeRef.current = performance.now(); rafRef.current = requestAnimationFrame(render)
    return () => { if (rafRef.current) cancelAnimationFrame(rafRef.current); canvas.removeEventListener('mousemove', onMove) }
  }, [ready, textureReady])

  return <canvas ref={canvasRef} className="mp-canvas" />
}
